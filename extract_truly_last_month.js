...
console.log('Extracting TRULY LAST month with sales (scanning all months)...\n');
...
const output = {
  ...
  dataSource: 'SPAR Sigma System - Department 2 (Butchery) [TRULY FINAL]',
...
fs.writeFileSync('butchery_all_products_last_month_TRULY_FINAL.json', JSON.stringify(output, null, 2));

console.log('\n✓ Successfully extracted ' + products.length + ' products [TRULY FINAL]\n');
...
console.log('\n--- RECENT Products (3-4 months) - First 20 ---');
...
const staleProds = products.filter(p => p.status === 'STALE').sort((a,b) => parseInt(a.code) - parseInt(b.code));
if (staleProds.length === 0) {
  console.log('  (None)');
} else {
  staleProds.slice(0, 10).forEach(p => {
    console.log('  ' + p.code.padEnd(8) + ' | ' + p.description.substring(0, 40).padEnd(40) + ' | ' + p.lastMonthReadable + ' | Qty: ' + Math.round(p.lastSalesQty));
  });
}
...
console.log('\n✓ Saved to: butchery_all_products_last_month_TRULY_FINAL.json');
