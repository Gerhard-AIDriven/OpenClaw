...
function buildRankMap(rs){
  const cand=rs.filter(r=>r[salesKey]!=null && r[salesKey]>0 && r[frontKey]!=null && r[actualKey]!=null)
    .slice()
...
// sort: best sales rank in 2026 first, then 2025, else code
...
for(const code of codeList){
  ...
  const move=(rank25!=null && rank26!=null)? (rank26-rank25) : null;
  results.push({
    shortCode: code,
    description: (a26?.desc ?? a25?.desc ?? null),
    rank2025: rank25,
    rank2026: rank26,
    movement_26_minus_25: move,
...
const payload={
  ...
  rankingMetric: 'Sales Excl. VAT (ranked descending)',
...
html += `<div class="muted">Input: ${esc(payload.inputFile)}<br/>Latest: ${esc(payload.latestDate)} vs Prev: ${esc(payload.prevDate)}<br/>Ranking metric: ${esc(payload.rankingMetric)}<br/>Filter: Sales Excl. VAT &gt; 0 (and Front GP% &amp; Actual Fin GP% present)</div>`;
html += '<table>';
html += '<thead><tr>';
...
html += '<th>Movement (26-25)</th>';
html += '</tr></thead><tbody>';

for(const r of results){
  const row=(v)=> (v==null? '—' : esc(v));
  html += '<tr>';
  ...
  html += `<td>${r.movement_26_minus_25==null? '—': r.movement_26_minus_25}</td>`;
  html += '</tr>';
}
