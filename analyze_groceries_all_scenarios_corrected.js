...
console.log('\n📊 SPAR Sigma - Groceries - All 3 Scenarios (CORRECTED MAPPING)\n');
...
// SCENARIO 3: Dormancy
const s3_active = allProducts.filter(p => p.lastMonth && p.lastQty > 0);
const refDate = new Date('2026-04-19');

s3_active.forEach(p => {
  if (p.lastMonth) {
    const parts = p.lastMonth.split('/');
    const d = parseInt(parts[0]);
    const m = parseInt(parts[1]);
    const y = parseInt(parts[2]);
    const date = new Date(2000 + y, m - 1, d);
    const diffTime = refDate - date;
    const monthsAgo = Math.floor(diffTime / (1000 * 60 * 60 * 24 * 30.44));
    
    if (monthsAgo <= 2) p.status = 'ACTIVE';
    else if (monthsAgo <= 4) p.status = 'RECENT';
    else if (monthsAgo <= 12) p.status = 'STALE';
    else p.status = 'DEAD STOCK';
  } else {
    p.status = 'DEAD STOCK';
  }
});
...
const s3_summary = {
  ACTIVE: s3_active.filter(p => p.status === 'ACTIVE').length,
  RECENT: s3_active.filter(p => p.status === 'RECENT').length,
  STALE: s3_active.filter(p => p.status === 'STALE').length,
  'DEAD STOCK': allProducts.filter(p => !p.lastMonth || !p.lastQty || p.lastQty === 0).length
};
...
console.log(`  ACTIVE: ${s3_summary.ACTIVE}`);
console.log(`  RECENT: ${s3_summary.RECENT}`);
console.log(`  STALE: ${s3_summary.STALE}`);
console.log(`  DEAD STOCK: ${s3_summary['DEAD STOCK']}\n`);
...
const outputFile = `${outputDir}/groceries_complete_analysis.json`;
