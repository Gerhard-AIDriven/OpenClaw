...
console.log('\n📊 Extracting Top 20 Good Performers - Final Metrics\n');
...
for (let c = 0; c <= range.e.c; c++) {
  ...
  if (value.match(/^\d{1,2}\/\d{1,2}\/\d{2}$/)) {
    monthLocations.push({
      ...
      monthCol: c,
      qtyCol: c + 1,
      gpCol: c + 4  // This is Gross Profit in Rand, not percentage
    });
  }
}
...
for (let r = 2; r <= range.e.r; r++) {
  ...
  for (let i = 0; i < monthLocations.length; i++) {
    ...
    const gpCell = ws[XLSX.utils.encode_cell({r, c: monthLocations[i].gpCol})]?.v || 0;
  ...
  // For GP slope (Gross Profit in Rand)
  sumY = 0;
  sumXY = 0;
  for (let i = 0; i < n; i++) {
  ...
  // Get most recent month with non-zero GP
  let currentGp = 0;
  for (let i = monthlyData.length - 1; i >= 0; i--) {
    if (monthlyData[i].gp > 0) {
      currentGp = monthlyData[i].gp;
      break;
    }
  }
  
  // Get total and avg qty
...
console.log('TOP 20 GOOD PERFORMERS');
console.log('=====================\n');
console.log('Rank | Code | Product Name | Qty/Mo | GP Growth/Mo | Current GP (R)');
console.log('-----|------|--------------|--------|--------------|---------------');

top20.forEach((p, i) => {
  const desc = p.description.substring(0, 28).padEnd(28);
  const qtyStr = p.qtySlope.toFixed(2).padEnd(6);
  const gpStr = p.gpSlope.toFixed(2).padEnd(12);
  const currentGpStr = p.currentGp.toFixed(0).padEnd(14);
  console.log(`${(i+1).toString().padStart(4)} | ${p.code.padEnd(6)} | ${desc} | ${qtyStr} | ${gpStr} | ${currentGpStr}`);
});
...
const jsonOutput = {
  top20: top20.map((p, i) => ({
    ...
    gpGrowthPerMonth: parseFloat(p.gpSlope.toFixed(2)),
    currentGpRand: Math.round(p.currentGp)
  }))
};
...
console.log('\nNote: GP Growth/mo = change in Gross Profit (R) per month');
console.log('      Current GP (R) = Gross Profit in Rand for most recent sales month');
