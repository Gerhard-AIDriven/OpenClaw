#!/usr/bin/env python3
"""
Napier Council Rates Scraper - ASSISTED MODE V2
...
def scrape_assisted():
    print("="*80)
    print("NAPIER COUNCIL RATES SCRAPER - ASSISTED MODE V2")
    print("="*80)
    print("\nINSTRUCTIONS:")
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            print(f"[INFO] Monitoring for property page (timeout: {max_wait_minutes} min)...")
            ...
            debug_html_path = 'due-diligence-mvp/debug_property_page.html'
            with open(debug_html_path, 'w', encoding='utf-8') as f:
                f.write(html)
            print(f"[DEBUG] Saved full HTML to: {debug_html_path}")
            ...
            if m:
                ...
                try:
                    ...
                    print(f"[WARN] Could not parse Land Value: {m.group(1)}")
            
            # Annual Rates - Multiple patterns
            m = re.search(r'Total Rates Levied</strong></td>\s*<td[^>]*>([\d,]+\.\d+)', html, re.I)
            if not m:
                m = re.search(r'Total Rates.*?(\$[\d,]+\.?\d*)', html, re.I)
            if not m:
                m = re.search(r'Annual Rates.*?(\$[\d,]+\.?\d*)', html, re.I)
            if m:
                val_text = m.group(1).replace('$','').replace(',','')
                try:
                    result['data']['annual_rates'] = float(val_text)
                    print(f"[OK] Annual Rates: ${result['data']['annual_rates']:,.2f}")
                except:
                    print(f"[WARN] Could not parse Rates: {m.group(1)}")
            
            # Legal Description
            m = re.search(r'<td>Legal Description&nbsp;</td>\s*<td[^>]*>(LOT\s+\d+\s+DP\s+\d+)', html, re.I)
            if not m:
                m = re.search(r'Legal Description.*?(LOT\s+\d+\s+DP\s+\d+)', html, re.I)
            if m:
            ...
            if not result['success']:
                print("\n⚠️  WARNING: No data could be extracted")
                print("Check the debug HTML file to see what's on the page")
                print(f"Debug file: {debug_html_path}")
...
if __name__ == '__main__':
    ...
    if result and result.get('success'):
        ...
        print("  python napier_scraper_assisted_v2.py")
    else:
