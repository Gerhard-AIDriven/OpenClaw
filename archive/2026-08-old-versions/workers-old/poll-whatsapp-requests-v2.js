...
/**
 * Send completion email with report link via Mailgun
 */
async function sendCompletionEmail(customerEmail, reportUrl, pdfUrl, address, orderId) {
  log('info', `Sending completion email to ${customerEmail}`);
  
  try {
    const mailgunUrl = `https://api.mailgun.net/v3/${MAILGUN_DOMAIN}/messages`;
    const subject = `✅ Your AI Driven Property Report is Ready!`;
...
<head>
  <style>
    ...
    .header { background: linear-gradient(135deg, #f7931e, #8b2fc9); color: white; padding: 30px; text-align: center; border-radius: 10px 10px 0 0; }
    .logo { font-size: 24px; font-weight: bold; margin-bottom: 10px; }
    .tagline { font-style: italic; opacity: 0.9; }
    .content { background: #f9f9f9; padding: 30px; border-radius: 0 0 10px 10px; }
    .btn { display: inline-block; background: #8b2fc9; color: white; padding: 15px 30px; text-decoration: none; border-radius: 8px; font-weight: bold; margin: 20px 0; }
    .btn:hover { background: #a855f7; }
    .details { background: white; padding: 20px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #f7931e; }
...
<body>
  <div class="container">
    <div class="header">
      <div class="logo">🧠 AI Driven</div>
      <div class="tagline">Practical AI for real businesses</div>
    </div>
    <div class="content">
      <h2>✅ Your Property Report is Ready!</h2>
      <p>Kia ora,</p>
      <p>Your AI Driven Due Diligence report for <strong>${address}</strong> has been generated and is ready for download.</p>
      
      <div class="details">
        <p><strong>📍 Property Address:</strong> ${address}</p>
        <p><strong>🆔 Order ID:</strong> ${orderId}</p>
        <p><strong>📊 Report Type:</strong> Tier 1 Due Diligence</p>
        <p><strong>✅ Status:</strong> Complete</p>
      </div>
      
      <div style="text-align: center;">
        <a href="${reportUrl}" class="btn" target="_blank">📊 View Your Property Report Online</a>
      </div>
      <p style="text-align: center; margin-top: 10px;"><em>The report includes interactive maps and a "Download PDF" button</em></p>
      
      <p style="margin-top: 20px;"><strong>What's included in your report:</strong></p>
      <ul>
        <li>Property details and address verification</li>
        <li>LINZ title information</li>
        <li>Natural hazards assessment</li>
        <li>Professional formatting with actionable insights</li>
      </ul>
      
      <p style="margin-top: 20px;"><strong>Next Steps:</strong></p>
      <ol>
        <li>Download and review your PDF report</li>
        <li>Use the information for your due diligence process</li>
        <li>Contact us if you have any questions or need clarification</li>
      </ol>
      
      <p style="margin-top: 20px;">Ngā mihi,<br><strong>The AI Driven Team</strong><br><a href="mailto:gerhard@aidriven.biz">gerhard@aidriven.biz</a><br>+64 27 461 0886</p>
    </div>
    <div class="footer">
      <p>This email was sent regarding your property due diligence request.<br>
      AI Driven - Practical AI for real businesses<br>
      © 2026 AI Driven. All rights reserved.</p>
    </div>
  </div>
</body>
</html>
...
Your AI Driven Due Diligence report for ${address} is ready!
...
WHAT'S INCLUDED:
- Property details and address verification
- LINZ title information
- Natural hazards assessment
- Professional formatting with actionable insights

NEXT STEPS:
1. Download and review your PDF report
2. Use the information for your due diligence process
3. Contact us if you have questions

Ngā mihi,
The AI Driven Team
gerhard@aidriven.biz | +64 27 461 0886

---
AI Driven - Practical AI for real businesses
© 2026 AI Driven. All rights reserved.
    `.trim();
    ...
    if (response.ok && result.id) {
      ...
      log('error', `Email send failed: ${result.message || response.statusText}`, result);
  ...
  try {
    ...
    for (const request of requests) {
      try {
        ...
        // Step 3: Send completion email with report link
        if (result.success) {
          // Extract customer email from request (handle different data formats)
          let customerEmail = '';
          
          // Check if it's a Google Form submission (email in name field due to mapping issue)
          if (request.customer && request.customer.name && request.customer.name.includes('@')) {
            customerEmail = request.customer.name; // Email is in name field
          } else if (request.customer && request.customer.email) {
          ...
          if (customerEmail && customerEmail.includes('@')) {
            ...
            if (emailResult.success) {
              log('info', `✅ Completion email sent successfully to ${customerEmail}`);
            } else {
              log('error', `⚠️ Failed to send completion email: ${emailResult.error}`);
            }
          } else {
            log('warn', `No valid email found for request ${request.id}`, { customer: request.customer });
          }
          
          // Step 4: Send WhatsApp message with report link
...
💡 Tip: If the link doesn't work immediately, wait 30 seconds and refresh the page.`;
          ...
          // Mark as completed regardless of WhatsApp success (email is primary)
          if (sendResult.success) {
            log('info', `✅ WhatsApp sent + Email sent for ${request.id}`);
            ...
            log('warn', `⚠️ WhatsApp failed but Email sent for ${request.id}: ${sendResult.error}`);
            // Mark as completed since email (primary) succeeded
            await updateRequestStatus(request.id, 'completed', {
              ...result,
              emailSent: true,
              messageSent: false,
