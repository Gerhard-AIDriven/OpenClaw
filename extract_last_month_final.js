...
const monthColumns = [
  'I', 'N', 'S', 'X', 'AC', 'AH', 'AM', 'AR', 'AW', 'BB',
  'BG', 'BL', 'BQ', 'BV', 'CA', 'CF', 'CK', 'CP', 'CU', 'CZ',
  'DA', 'DJ', 'DO', 'DT', 'DY', 'ED', 'EI', 'EN', 'ES', 'EX',
  'FC', 'FH', 'FM', 'FR', 'FW', 'GB', 'GG', 'GL', 'GQ', 'GV',
  'HA', 'HF', 'HK', 'HP', 'HU', 'HZ', 'IE', 'IJ', 'IO'
];
...
// Sales Qty columns (every 5th column starting from column J = index 1 in the month block)
// Column J is Sales Qty for March 2022, Column P is Sales Qty for April 2022, etc.
...
products.filter(p => p.status === 'ACTIVE').slice(0, 10).forEach(p => {
  console.log('  ' + p.code + ' | ' + p.description + ' | ' + p.lastMonthReadable + ' | Qty: ' + p.lastSalesQty);
});
...
console.log('\n✓ Saved to: butchery_all_products_last_month.json');
