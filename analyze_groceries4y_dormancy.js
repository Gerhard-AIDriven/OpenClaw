...
console.log('\n📊 SPAR Sigma - Groceries4y Department - Dormancy Analysis (Scenario 3)\n');

const inputFile = 'Spar/Data Extracts/gws groceries4y.xls';
const outputJsonFile = 'Spar/Data Extracts/groceries4y_product_status.json';
const outputHtmlFile = 'Spar/Data Extracts/groceries4y_product_status_report.html';

console.log(`Input File: ${inputFile}`);
console.log(`Reference Date: 19 April 2026`);
console.log(`Analysis Period: 1 March 2022 - 1 April 2026 (38 months)\n`);
...
for (let r = 2; r <= range.e.r; r++) {
  ...
  if (lastMonth) {
    ...
    const refDate = new Date('2026-04-19');
...
let recentHtml = products.filter(p => p.status === 'RECENT').slice(0, 20).map(p =>
...
let staleHtml = products.filter(p => p.status === 'STALE').slice(0, 20).map(p =>
...
let deadHtml = products.filter(p => p.status === 'DEAD STOCK').slice(0, 20).map(p =>
...
const recentMore = summary.RECENT > 20 ? '<p style="text-align: center; color: #999; margin-top: 10px;">... and ' + (summary.RECENT - 20) + ' more in RECENT</p>' : '';
const staleMore = summary.STALE > 20 ? '<p style="text-align: center; color: #999; margin-top: 10px;">... and ' + (summary.STALE - 20) + ' more in STALE</p>' : '';
const deadMore = summary['DEAD STOCK'] > 20 ? '<p style="text-align: center; color: #999; margin-top: 10px;">... and ' + (summary['DEAD STOCK'] - 20) + ' more in DEAD STOCK</p>' : '';
...
<head>
    ...
    <title>SPAR Groceries4y - Product Status Report</title>
    <style>
...
<body>
    <div class="container">
        <div class="header">
            <h1>📊 Groceries4y Department - Product Status Analysis</h1>
            <p>Last Month with Sales Activity — Dormancy-Based Classification</p>
        </div>
        
        <div class="metadata">
            <div><strong>Report Date:</strong> 19 April 2026</div>
            <div><strong>Analysis Period:</strong> 1 March 2022 – 1 April 2026 (38 months)</div>
        ...
        <div class="footer">
            <p>Report Generated: 19 April 2026 | SPAR Groceries4y Department | Product Status Analysis (Scenario 3)</p>
        </div>
    </div>
</body>
</html>`;
