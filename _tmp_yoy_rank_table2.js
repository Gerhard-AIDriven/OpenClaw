...
const header=['Sub-Dept','Rank 2025','Rank 2026','YoY move (26-25)'];
const data=codes.map(code=>{
  ...
  return [code,String(rank25),String(rank26),String(move)];
});

const widths=header.map((_,j)=>Math.max(header[j].length, ...data.map(r=>String(r[j]).length)));
function pad(s,w){ s=String(s); if(s.length>=w) return s; return s+' '.repeat(w-s.length);} 

let out='';
out+=header.map((h,j)=>pad(h,widths[j])).join('  ')+'\n';
for(const row of data){ out+=row.map((v,j)=>pad(v,widths[j])).join('  ')+'\n'; }

out=`YoY ranking (Actual Fin GP%), showing departments with Sales Excl. VAT > 0 in 2026\nLatest: ${dateStr(latest)} | Prev: ${dateStr(prev)} | Candidates: 2026=${salesPos2026.length}, 2025=${salesPos2025.length}.\n\n`+out;
console.log(out);
